...
The source container remains stopped and recoverable until Compose reconciles node 1. If adoption fails before the stable copy is verified, the migration helper restores the original Kafka and Connect container states and the deployment cleanup does not start the new topology against an empty volume. The migration never formats the adopted node-1 volume, deletes a source log tree, resets connector offsets, or substitutes a new cluster ID. Any identity, copy, feature-level, observer, reassignment, or ISR mismatch stops delivery for diagnosis. Once dynamic membership and RF3 are established, recovery operates on the three-node cluster; it is not a destructive return to a fresh single-node cluster. RF3 retains approximately three copies of the Kafka log plus replication overhead. The previous anonymous source volume is not deleted by this migration, so the recovery window can temporarily consume a fourth copy; retiring that source requires a separate, explicitly approved operation after the new cluster is reviewed. VM disk usage must be monitored as retention grows.
