...
The failover smoke test allows 90 polling attempts for each saga transition while clients recover; ordinary smoke tests retain 30 attempts (configurable with `SMOKE_WAIT_ATTEMPTS`). Each unsuccessful attempt waits one second. Kafka brokers start without waiting for peer health so an existing three-voter quorum can recover after the entire stack stops. Application startup still waits for cluster initialization and verification.
